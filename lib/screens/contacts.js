...
export default class Contacts extends React.Component {
  constructor(props) {
    ...
    this.deleteKontak = this.deleteButtonHandler.bind(this);
  ...
  componentDidMount() {
    this.fetchKontak();
    this.unsubscribe = this.navigation.addListener('focus', () => {this.fetchKontak()});
  }
  ...
  async pushKontak() {
    try {
      //Convert kontak yang sekarang jadi string
  ...
  kontakItemHandler(kontakPilihan) {
  ...
  kontakItemLongHandler(kontakPilihan) {
    this.setState({
      dialogVisible: true,
  ...
  addButtonHandler() {
    if (this.state.namaAdd == '') {
      ...
      //Bikin array baru yang isinya kontak sekarang ditambah dengan kontak yang baru
      const newKontak = [
        ...this.state.kontak,
        {
      ...
      //Update state supaya list terupdate
      this.setState(
        {
        ...
        //Setelah itu push ke storage kalo statenya udah berubah
        this.pushKontak,
      );
    }
  }
  ...
  kontakItem({item}) {
    ...
    return (
      <TouchableOpacity
        ...
        key={item.key}
        onPress={() => this.kontakItemHandler(item)}
        onLongPress={() => this.kontakItemLongHandler(item)}>
        <View
